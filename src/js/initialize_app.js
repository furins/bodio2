...
app.controller('appController', ['$scope', '$http', '$state', '$location',
        ...
        document.addEventListener("deviceready", function () {
            ...
            if (codice_iscr !== null) {
                if (localStorage.getItem('dati_iscrizione') === null) {
                    ...
                    var url = scc_customizations.server_club_url + "/api/v1/iscrizioni_codice/" + codice_iscr + "/?username=indalo&api_key=6ca8ed17381d56bd5540484722c2422a60d34863"; // development
                    $http.get(url).then(function (response) {
                        ...
                        console.log("L'utente è iscritto al club, ho aggiornato i dati.");
                        console.log(JSON.parse(localStorage.getItem('dati_iscrizione')));
                        localStorage.setItem('iscritto_al_club', true);
                    }, function (response) {
                        if (response.data === 'More than one resource is found at this URI.') {
                            ...
                            var url = scc_customizations.server_club_url +
                                "/api/v1/iscrizioni_nome/" +
                                codice_iscr +
                                "/?nome="+nome+"&cognome="+cognome+"&username=indalo&api_key=6ca8ed17381d56bd5540484722c2422a60d34863";
                            $http.get(url).then(function (response) {
                                console.log('codice_card_aggiornato');
                                localStorage.setItem('iscritto', response.data.codice);
                                localStorage.setItem('iscritto_nome', response.data.nome + ' ' + response.data.cognome);
                                localStorage.setItem('dati_iscrizione', JSON.stringify(response.data));
                                console.log("L'utente ora è univoco, ho aggiornato i dati.");
                                console.log(response);
                                console.log(JSON.parse(localStorage.getItem('dati_iscrizione')));
                                localStorage.setItem('iscritto_al_club', true);
                            }, function (response) {
                        ...
                        console.log("Impossibile capire se l'utente è iscritto al club.");
                        console.log('errore');
                        console.log(response);
                        localStorage.setItem('iscritto_al_club', false);
                    });
